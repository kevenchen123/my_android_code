...
public abstract class ScrollPickerView<T> extends View {
    ...
    private float mMoveLength = 0; // item移动长度，负数表示向上移动，正数表示向下移动 （相对于当前选中项mSelected的偏移，不超过一个mItemSize）
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 只绘制可见的item
        int length = Math.max(mCenterPosition + 1, mVisibleItemCount - mCenterPosition);    //中间开始算两边的数量，再加一
        int position;
        ...
        for (int i = start; i >= 1; i--) { // 先从远离中间位置的item绘制，当item内容偏大时，较近的item覆盖在较远的上面
            ...
            if (mDrawAllItem || i <= mCenterPosition + 1) {  // 上面的items,相对位置为 -i
                position = mSelected - i < 0 ? mData.size() + mSelected - i : mSelected - i;
                // 传入位置信息，绘制item
                if (mIsCirculation) {
            ...
            if (mDrawAllItem || i <= mVisibleItemCount - mCenterPosition) {  // 下面的items,相对位置为 i
                position = mSelected + i >= mData.size() ? mSelected + i - mData.size() : mSelected + i;
                // 传入位置信息，绘制item
                if (mIsCirculation) {
                    drawItem(canvas, mData, position, i, mMoveLength, mCenterPoint + mMoveLength + i * mItemSize);
     ...
     * @param moveLength 中间item滚动的距离，moveLength<0则表示向上（左）滚动的距离，moveLength＞0则表示向下（右）滚动的距离
    ...
    /**
     * @param curr ValueAnimator当前值
     * @param end  ValueAnimator最后值
     * @param rate 比率
     */
    ...
    private void checkCirculation() {
        if (mMoveLength >= mItemSize) { // 向下滑动，向右滑动 （向数据头移动）
            // 该次滚动距离中越过的item数量
        ...
        } else if (mMoveLength <= -mItemSize) { // 向上滑动，向左滑动 （向数据尾移动）
            // 该次滚动距离中越过的item数量
            int span = (int) (-mMoveLength / mItemSize);
            mSelected += span;
