 ...
 * @date 2016.1.11
 */
public abstract class ScrollPickerView2<T> extends View {
    ...
    private int mLastSelected; // 上次选中的item下标
    ...
    @RequiresApi(api = Build.VERSION_CODES.CUPCAKE)
    public ScrollPickerView2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void notifySelected(final boolean trigger) {
        if (mListener != null) {
            ...
            post(new Runnable() {
                @Override
                public void run() {
                    if (mLastSelected != mSelected && trigger) {
                        mListener.onSelected(ScrollPickerView2.this, mSelected);
                    }
    ...
    public interface OnSelectedListener {
        void onSelected(ScrollPickerView2 scrollPickerView, int position);
    }
