 ...
 * https://www.jianshu.com/p/0e25a10cb9f5
 * --
 * <p>
...
public class WaveView1 extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        if (attrs != null) {
            //attars
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.WaveView1);

            int bgColor = typedArray.getColor(R.styleable.WaveView1_backgroudColor, BG_COLOR);
            int pColor = typedArray.getColor(R.styleable.WaveView1_progressColor, WAVE_COLOR);
            int aMax = typedArray.getInt(R.styleable.WaveView1_max, (int) max);
            int aP = typedArray.getInteger(R.styleable.WaveView1_progress, (int) progress);
            BG_COLOR = bgColor;
            WAVE_COLOR = pColor;
            max = aMax;
            progress = aP;
        ...
        VIEW_WIDTH = DisplayUtils.dpToPx(300);
        VIEW_HEIGHT = DisplayUtils.dpToPx(300);
...
//        Log.d(TAG,"onMeasure " +isHasWindowFocus);
    ...
    private int getRealWidthMeasureSpec(int widthMeasureSpec) {
        ...
        if (mode == MeasureSpec.AT_MOST) {
            Log.i(TAG, "AT_MOST width :" + DisplayUtils.pxToDp(widthSize));//warp
//            VIEW_WIDTH = widthSize;
        } else if (mode == MeasureSpec.EXACTLY) {
            Log.i(TAG, "EXACTLY width :" + DisplayUtils.pxToDp(widthSize));
            VIEW_WIDTH = widthSize;
        } else if (mode == MeasureSpec.UNSPECIFIED) {
            Log.i(TAG, "UNSPECIFIED width :" + DisplayUtils.pxToDp(widthSize));
//            VIEW_WIDTH = VIEW_WIDTH_TMP;
    ...
    private int getRealHeightMeasureSpec(int heightMeasureSpec) {
        ...
        if (mode == MeasureSpec.AT_MOST) {
            Log.i(TAG, "AT_MOST heitht :" + DisplayUtils.pxToDp(heightSize));
//            VIEW_HEIGHT = heightSize;
        } else if (mode == MeasureSpec.EXACTLY) {
            Log.i(TAG, "EXACTLY heitht :" + DisplayUtils.pxToDp(heightSize));
            VIEW_HEIGHT = heightSize;
        } else if (mode == MeasureSpec.UNSPECIFIED) {
            Log.i(TAG, "UNSPECIFIED heitht :" + DisplayUtils.pxToDp(heightSize));
...
//            WAVE_HEIGHT = (float) (VIEW_HEIGHT / 50);
            ...
            int startX = 0;
            Log.i(TAG, "begin point (" + DisplayUtils.pxToDp(startX) + " , " + DisplayUtils.pxToDp(dy) + ")");
            ...
            SPEED_NORMAL = (DisplayUtils.pxToDp(VIEW_WIDTH) / 20);
...
//                resetWave();
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        Log.d(TAG," onWindowFocusChanged " + hasWindowFocus + " isDone " + isDone);
        if (!isDone) {
            if (!isStartAnimation) {
            ...
            Log.d(TAG," onWindowFocusChanged " + isDone);
            if (isHasWindowFocus) {
                doneAnimation();
            }
        }
    ...
    private void flowingAnimation() {
        flowingAnimato = ObjectAnimator.ofFloat(this, "wave", 0, 100).setDuration(100);
        flowingAnimato.setRepeatCount(Animation.INFINITE);
        flowingAnimato.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                Log.i(TAG, "wave  onAnimationUpdate" + dx);
                dx = dx + speed;
    ...
    private void riseAnimation() {
        ...
        isMeasure = true;
        Log.i(TAG, "move " + "dy " + dy);
        if (dy > 0) {
            float offset = updateDyData();
            Log.i(TAG, "move s " + offset + "and sum_dy" + sum_dy);
            reiseAnimator = ValueAnimator.ofFloat(0, offset).setDuration(500);
