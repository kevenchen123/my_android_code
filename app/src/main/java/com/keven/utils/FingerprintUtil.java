...
public class FingerprintUtil {
    ...
    public static void callFingerPrint(Context context, final OnCallBackListenr listener) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            ...
            if (!manager.isHardwareDetected()) { //判断设备是否支持
                if (listener != null)
            ...
            if (!manager.hasEnrolledFingerprints()) { //判断设备是否已经注册过指纹
                if (listener != null)
            ...
            android.os.CancellationSignal cancellationSignal = new android.os.CancellationSignal(); //必须重新实例化，否则cancel 过一次就不能再使用了
