...
package com.keven.security.util;
...
public final class RSAUtils
{
	...
	private final static String CIPHER = "RSA/ECB/OAEPWithSHA1AndMGF1Padding";//"RSA/ECB/PKCS1Padding";
	/**
	 * 加密Key的长度等于1024
	 */
	 ...
	 * 加密时小于245即可
	 */
	public static int encodeLen = 240;//(DEFAULT_KEY_SIZE / 8) - 11;
	...
	public static byte[] encryptData(byte[] data, PublicKey publicKey)
	{
		...
		for (int i = 0; i < data.length; i += encodeLen) {
			...
			try {
				doFinal = encryptByPublicKey(subarray, publicKey);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public static byte[] decryptData(byte[] encryptedData, PrivateKey privateKey)
	{
		...
		for (int i = 0; i < encryptedData.length; i += decodeLen) {
			byte[] subarray = ArrayUtils.subarray(encryptedData, i, i + decodeLen);
			byte[] doFinal = new byte[0];
			try {
				doFinal = decryptByPrivateKey(subarray, privateKey);
			} catch (Exception e) {
				e.printStackTrace();
			}
